...
describe("release workflows", () => {
  ...
  it("requires Vela CLI for every beta desktop packaging target", async () => {
    ...
    expect(buildMac).not.toContain("build_args+=(--require-vela-cli)");
    expect(buildMac).not.toContain("update_args+=(--require-vela-cli)");
    ...
    expect(buildWin).not.toContain('$buildArgs += "--require-vela-cli"');
    expect(buildWin).not.toContain('$updateArgs += "--require-vela-cli"');
