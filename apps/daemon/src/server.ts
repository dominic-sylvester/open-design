...
export async function startServer({
  ...
  // Local-only workspace authority: explicit request headers are the source of truth.
  const verifyExplicitWorkspaceRequestContext = async (input: {
    req: any;
    requireTeam?: boolean;
  }): Promise<import('./workspace/workspace-resource-mutation.js').WorkspaceRequestAuthorityResult> => {
    ...
    return {
      ...
      context: {
        ...
        canShareProjects: claimed.canShareProjects,
        canWriteSyncedFiles: claimed.canWriteSyncedFiles,
      },
    };
  };
  ...
  async function resolveDesignSystemWorkspaceContext(
    req: any,
  ): Promise<import('./workspace/workspace-resource-mutation.js').WorkspaceResourceContext | null> {
  ...
  const createWorkspaceOwnedDesignSystemForContext = (
    ...
    context: import('./workspace/workspace-resource-mutation.js').WorkspaceResourceContext | null,
  ...
  const revokedTeamProjectMirrors = new Set<string>();
  const projectIsUnmaterializedSharedPlaceholder = (_projectId: string): boolean => false;
  const configuredAmrEnv = () => ({});
  const collabSyncRoutes = createLocalCollabSyncStub();
  const collabCloud = null;
